...
[为什么使用 OSpec Lite](#为什么使用-ospec-lite) | [快速开始](#快速开始) | [工作汇报](#工作汇报) | [会生成什么](#会生成什么) | [Profiles](#profiles) | [Codex 插件](#codex-插件) | [Plugins 模块](#plugins-模块) | [命令参考](#命令参考) | [开发](#开发)
...
`oslite report` 会为已初始化仓库汇总当前 OSpec Lite 状态，不会修改 change、bug 或 docs 状态。报告内容来自：

- `.oslite/changes/active/*` 里的 open changes
- 最近归档的 changes
- `.oslite/bugs/active-bugs.md` 里的 open bugs
- 最近 applied 的 bugs
- 需要人工复核 generated suggestions 的 docs
...
如果想留下可归档、可附加、可 review 的文件，运行：

```text
oslite report write . --cadence weekly
```

它会在 `.oslite/reports/<cadence>/` 下写入 Markdown 和 JSON，例如：
...
如果要持续产出 report artifacts，先创建仓库本地 schedule：

```text
oslite report schedule . --cadence daily
```

然后让 cron、CI 或 agent automation 反复调用幂等 runner：

```text
oslite report run .
```

runner 只会在当前 daily 或 weekly period 到期时产出报告。Daily period 使用 UTC 日期，weekly period 使用 ISO week key；如果确实要覆盖当前 period artifact，可以加 `--force`。
