...
- 如果用户要求“用 unity-tolua-hall profile 初始化”，且仓库还没有 `.oslite/config.json`，先推断项目名并向用户确认；如果用户给了明确的工程路径，就在该 `<repo-root>` 上运行 `oslite init --profile unity-tolua-hall --project-name "<项目名>" --bootstrap-agent claude-code "<repo-root>"`；如果当前环境并不确定，则改用 `none`。否则默认当前工作目录就是目标工程根目录。
