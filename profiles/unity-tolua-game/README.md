...
```text
工程路径是 XXXX（建议填绝对路径）。帮我用 unity-tolua-game 的 profile 去 init ospec-lite；如果这个工程还没初始化，请先推断项目名并向我确认。然后先补 evidence-map，再补正式项目文档，最后跑 oslite docs verify XXXX。
```
...
```text
Project path is XXXX (prefer an absolute path). Help me use the unity-tolua-game profile to init ospec-lite in that repo. If the repo is not initialized yet, infer the project name and ask me to confirm it first. Then fill evidence-map before the final docs, and finish with oslite docs verify XXXX.
```
